<div
    x-data="{
        ...
        setTab(tab) {
            ...
            this.activeTab = tab;
            localStorage.setItem('glass.status-filter', tab);
        },
        openPromptModal() {
            ...
            this.$dispatch('openModal', { component: 'modals.prompt.detail-prompt', arguments: { productSlug: 'glass' } });
        ...
        openImportModal() {
            ...
            this.$dispatch('openModal', { component: 'modals.glass.excel-import-glass' });
        ...
        openAddModal() {
            ...
            this.$dispatch('openModal', { component: 'modals.glass.add-product-design' });
            window.setTimeout(() => this.addOpening = false, 900);
        }
    }"
    x-init="
        if (! window.__glassBeforeUnloadGuardInstalled) {
            window.__glassBeforeUnloadGuardInstalled = true;
            window.__glassGenerationCount = window.__glassGenerationCount || 0;

            window.addEventListener('glass-generation-started', () => {
                window.__glassGenerationCount = (window.__glassGenerationCount || 0) + 1;
            });

            window.addEventListener('glass-generation-finished', () => {
                window.__glassGenerationCount = Math.max(0, (window.__glassGenerationCount || 0) - 1);
            });

            window.addEventListener('beforeunload', (event) => {
                if ((window.__glassGenerationCount || 0) <= 0) {
                    return;
                }
...
>
        ...
        <div class="mb-4 overflow-visible rounded-lg border border-slate-200 bg-white shadow-sm">
            <div class="flex flex-col gap-3 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <h1 class="text-base font-bold text-slate-950">Glass Workspace</h1>
                        <p class="mt-0.5 text-xs text-slate-500">Quản lý quy trình tạo glass</p>
                    </div>
                </div>
                    ...
                    <div x-data="{ open: false }" class="relative z-20">
                        ...
                        <div x-cloak x-show="open" x-transition.origin.top.right x-on:click.outside="open = false" class="absolute right-0 top-full mt-2 w-52 rounded-lg border border-slate-200 bg-white p-1 shadow-2xl">
                            @if (! array_key_exists('v98store', $providerOptions))
                                <button type="button" x-on:click="open = false" wire:click="$dispatch('openModal', { component: 'modals.ai.change-v98-store-key', arguments: { functionKey: 'glass' } })" class="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-xs font-bold text-slate-700 hover:bg-slate-100"><span class="text-base leading-none">+</span> API v98</button>
                            @endif
                            @if (! array_key_exists('cheapkeyai', $providerOptions))
                                <button type="button" x-on:click="open = false" wire:click="$dispatch('openModal', { component: 'modals.ai.change-cheap-key-ai-key', arguments: { functionKey: 'glass' } })" class="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-xs font-bold text-slate-700 hover:bg-slate-100"><span class="text-base leading-none">+</span> API CheapKeyAI</button>
                            @endif
                        </div>
                    </div>
                    @if (($selectedAiProvider ?? null) === 'v98store' || ($selectedAiProvider ?? null) === 'cheapkeyai')
                        <button type="button" wire:click="$dispatch('openModal', { component: '{{ ($selectedAiProvider ?? null) === 'cheapkeyai' ? 'modals.ai.change-cheap-key-ai-key' : 'modals.ai.change-v98-store-key' }}', arguments: { functionKey: 'glass' } })" class="inline-flex h-9 items-center justify-center rounded-md border border-amber-200 bg-amber-50 px-3 text-xs font-bold text-amber-700">{{ ($selectedAiProvider ?? null) === 'cheapkeyai' ? 'Change API CheapKeyAI' : 'Change API v98' }}</button>
        ...
        <div class="mt-4">
            @foreach (['all', 'unapproved', 'approved'] as $status)
                <div
                    ...
                    x-cloak
                >
                    <livewire:pages.glass.glass-status-panel
                        :status="$status"
                        :per-page="$perPage"
                        :search="$search"
    ...
    <button
        ...
        aria-label="Them glass item"
        title="Them glass item"
    >
    ...
    <livewire:modals.ai.change-cheap-key-ai-key function-key="glass" />
