                    ...
                    <div class="max-h-[70vh] space-y-5 overflow-y-auto p-5 md:p-6">
                        @if (! filled($sheetUrl))
                            <form wire:submit.prevent="save" class="space-y-4">
                                <div>
                                    <label for="suncatcher-sheet-url" class="mb-2 block text-sm font-medium text-slate-900">Sheet URL</label>
                                    <x-input id="suncatcher-sheet-url" wire:model="sheetUrl" type="url" class="block w-full" placeholder="https://docs.google.com/spreadsheets/d/..." autofocus />
                                        ...
                                        <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
                                            <thead class="sticky top-0 bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                                                <tr>
                                                    ...
                                                    <th class="min-w-72 px-4 py-3">Link Product</th>
                                                    <th class="min-w-72 px-4 py-3">Link Ipnut Main Image</th>
                                                    <th class="min-w-72 px-4 py-3">Link Main Image</th>
                                            ...
                                            <tbody class="divide-y divide-slate-100 text-slate-700">
                                                @foreach ($rows as $index => $row)
                                                    <tr wire:key="suncatcher-sheet-row-{{ $row['row'] ?? $index }}-{{ $row['sku'] ?? $index }}">
                                                        ...
                                                        <td class="min-w-48 max-w-64 px-4 py-3">
                                                            <div x-data="{ expanded: false, value: @js($row['keyword_phrase'] ?? '') }" class="text-xs text-slate-700">
                                                                <span class="break-words" x-text="expanded ? value : (value.length > 80 ? value.slice(0, 80) + '...' : value)"></span>
                                                                <button x-show="value.length > 80" type="button" x-on:click="expanded = ! expanded" class="ml-1 text-[11px] font-semibold text-indigo-600 hover:text-indigo-700" x-text="expanded ? 'Thu gọn' : 'Xem thêm'"></button>
                                                            </div>
                                                        </td>
                                                        <td class="max-w-80 px-4 py-3">
                                                            <div x-data="{ expanded: false, value: @js($row['product_link'] ?? '') }" class="text-xs text-slate-700">
                                                                <a href="{{ $row['product_link'] }}" target="_blank" rel="noopener noreferrer" class="break-all text-indigo-600 hover:underline" x-text="expanded ? value : (value.length > 80 ? value.slice(0, 80) + '...' : value)"></a>
                                                                <button x-show="value.length > 80" type="button" x-on:click="expanded = ! expanded" class="ml-1 text-[11px] font-semibold text-indigo-600 hover:text-indigo-700" x-text="expanded ? 'Thu gọn' : 'Xem thêm'"></button>
                                                            </div>
                                                        </td>
                                                        <td class="max-w-80 px-4 py-3">
                                                            <div x-data="{ expanded: false, value: @js($row['input_main_image'] ?? '') }" class="text-xs text-slate-700">
                                                                <a href="{{ $row['input_main_image'] }}" target="_blank" rel="noopener noreferrer" class="break-all text-indigo-600 hover:underline" x-text="expanded ? value : (value.length > 80 ? value.slice(0, 80) + '...' : value)"></a>
                                                                <button x-show="value.length > 80" type="button" x-on:click="expanded = ! expanded" class="ml-1 text-[11px] font-semibold text-indigo-600 hover:text-indigo-700" x-text="expanded ? 'Thu gọn' : 'Xem thêm'"></button>
                                                            </div>
                                                        </td>
                                                        <td class="max-w-80 px-4 py-3">
                                                            @if(filled($row['main_image']))
                                                                <div x-data="{ expanded: false, value: @js($row['main_image'] ?? '') }" class="text-xs text-slate-700">
                                                                    <a href="{{ $row['main_image'] }}" target="_blank" rel="noopener noreferrer" class="break-all text-indigo-600 hover:underline" x-text="expanded ? value : (value.length > 80 ? value.slice(0, 80) + '...' : value)"></a>
                                                                    <button x-show="value.length > 80" type="button" x-on:click="expanded = ! expanded" class="ml-1 text-[11px] font-semibold text-indigo-600 hover:text-indigo-700" x-text="expanded ? 'Thu gọn' : 'Xem thêm'"></button>
                                                                </div>
                                                            @else
                        ...
                        @if ($rowErrors !== [])
                            <div class="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                                <div class="flex items-center justify-between gap-3">
                                    <div>
                                        <p class="text-sm font-semibold text-amber-900">{{ count($rowErrors) }} dòng có lỗi</p>
                                        <p class="mt-1 text-xs text-amber-800">Chi tiết lỗi hiển thị bên dưới để user kiểm tra và sửa.</p>
                                    </div>
                                    <button type="button" wire:click="toggleErrors" class="rounded-xl bg-white px-3 py-2 text-xs font-medium text-amber-800 transition hover:bg-amber-100">{{ $showErrors ? 'Ẩn lỗi' : 'Xem lỗi' }}</button>
                                </div>
