        ...
        <div class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/40 p-4 md:p-6">
            <div class="relative mx-auto mt-6 w-full max-w-7xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.12)]">
                ...
                <div class="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-5 md:px-6">
                    <div>
                        <h2 class="text-lg font-semibold tracking-tight text-slate-900">Import Glass Excel</h2>
                        ...
                        <p class="mt-2 text-sm text-slate-600">
                            Template:
                            <a href="{{ asset('templates/glass-import-template.xlsx') }}" download="glass-import-template.xlsx" class="font-semibold text-emerald-600 underline decoration-emerald-200 underline-offset-4 transition hover:text-emerald-700">
                                glass-import-template.xlsx
                            </a>
                        </p>
                    </div>
                ...
                <div class="space-y-4 px-5 py-5 md:px-6">
                    @if ($rows === [] && $rowErrors === [])
                        <label for="glass-import-excel" class="relative block cursor-pointer overflow-hidden rounded-2xl border border-dashed border-slate-300 bg-white px-5 py-8 text-center shadow-sm transition hover:border-slate-400 hover:bg-slate-50">
                            ...
                            <input id="glass-import-excel" type="file" wire:model.live="excelFile" accept=".xlsx,.xls,.csv,.txt" class="sr-only">
                        </label>
                    @endif
                                ...
                                <table class="min-w-full divide-y divide-slate-200 text-left text-sm">
                                    ...
                                    <tbody class="divide-y divide-slate-100 bg-white">
                                        @foreach ($rows as $index => $row)
                                            ...
                                            <tr wire:key="glass-import-row-{{ $row['row'] }}-{{ $row['sku'] ?? $index }}">
