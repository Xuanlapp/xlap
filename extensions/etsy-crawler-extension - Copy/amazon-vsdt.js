...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'AMAZON_START_JOB' || message.type === 'START_VSDT') {
    ...
    const payload = {
      keywords: [...new Set((message.payload.keywords || []).map((item) => String(item).trim()).filter(Boolean))],
      sellerSearch: String(message.payload.sellerSearch || 'sticker').trim(),
      topPerSeller: Math.max(1, Math.min(20, Number(message.payload.topPerSeller) || 5)),
      runCerebro: Boolean(message.payload.runCerebro),
      heliumAccountId: String(message.payload.heliumAccountId || '').trim(),
      requestId: String(message.requestId || message.payload.requestId || `amazon_${Date.now()}`)
    };
  ...
  if (message.type === 'START_CEREBRO_TEST') {
    ...
    const payload = {
      asins: [...new Set((message.payload.asins || [])
