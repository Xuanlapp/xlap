...
class ImportExcelOrnament extends Component
{
    use WithFileUploads;
    ...
    /**
     * @var array<int, array{row: int, product_link: string, main_image: string, keyword: string, status: string}>
     */
    public array $rows = [];
    ...
    {
        if ($component !== 'modals.ornament-amazon-two.import-excel-ornament') {
            return;
        }
    ...
    /**
     * Import all preview rows as Ornament Amazon 2 items.
     */
    public function save(): void
    {
        ...
        $this->dispatch('toast', type: 'success', title: 'Import success!', message: " thm {$imported} item Ornament Amazon 2.");
    }
    ...
    {
        return view('livewire.modals.ornament-amazon-two.import-excel-ornament');
    }

    /**
     * @return array<int, array{row: int, product_link: string, main_image: string, keyword: string, status: string}>
     */
    private function parseRows(TemporaryUploadedFile $file): array
    {
        ...
        for ($index = 1; $index < count($rows); $index++) {
            ...
            $mainImage = trim((string) ($rows[$index][$headerIndexes['main_image']] ?? ''));
            ...
            if ($mainImage === '') {
                $this->rowErrors[] = ['row' => $rowNumber, 'message' => 'Thi?u Link Main Image.'];
                continue;
            }
            ...
            if (! $this->isImageUrl($mainImage)) {
                $this->rowErrors[] = ['row' => $rowNumber, 'message' => 'Link Main Image khng h?p l?.'];
                continue;
            }
     ...
     * @return array{product_link?: int, main_image?: int}
     */
