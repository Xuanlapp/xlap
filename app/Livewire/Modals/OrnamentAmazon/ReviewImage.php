...
class ReviewImage extends Component
{
    ...
    public function generateOrnamentAmazonMockupImage(): void
    {
        if ($this->action !== 'ornament-amazon-custom-image' || ! $this->assetId || ! $this->editTarget) {
            return;
        }
        ...
        if (! $slot) {
            ...
            $this->dispatch('ornament-amazon-preview-mockup-generation-finished', assetId: $this->assetId, slot: null, ok: false, message: 'Slot mockup khong hop le.');
        ...
        try {
            ...
            app(ActivityLogService::class)->record(
                event: 'ornament_amazon.preview_mockup_generated',
                description: 'User generated one Ornament Amazon mockup from preview.',
                subject: $asset,
                properties: ['item_number' => $asset->item_number, 'target' => $this->editTarget, 'slot' => $slot, 'provider' => $this->modalProviderKey],
            );
        } catch (RuntimeException $exception) {
            $this->reportUserActionError($exception, 'ornament_amazon.preview_generate_mockup', [
                'asset_id' => $this->assetId,
                'target' => $this->editTarget,
                'slot' => $slot,
            ]);
            $this->dispatch('ornament-amazon-generation-finished');
            $this->dispatch('ornament-amazon-preview-mockup-generation-finished', assetId: $this->assetId, slot: $slot, ok: false, message: $exception->getMessage());
            ...
            Log::error('Ornament Amazon preview mockup generation failed unexpectedly.', [
                'asset_id' => $this->assetId,
                'target' => $this->editTarget,
                'slot' => $slot,
            ...
            $this->dispatch('ornament-amazon-generation-finished');
            $this->dispatch('ornament-amazon-preview-mockup-generation-finished', assetId: $this->assetId, slot: $slot, ok: false, message: 'Loi he thong khi tao anh mockup.');
        ...
        $this->dispatch('ornament-amazon-product-design-updated', assetId: $asset->id);
        $this->dispatch('ornament-amazon-generation-finished');
        $this->dispatch('ornament-amazon-preview-mockup-generation-finished', assetId: $asset->id, slot: $slot, ok: true, url: $updatedUrl);
    ...
    /**
     * Customize the currently opened Ornament Amazon Create Master or mockup image.
     */
    public function customizeOrnamentAmazonImage(): void
    {
        if ($this->action !== 'ornament-amazon-custom-image' || ! $this->assetId || ! $this->original || ! $this->editTarget) {
            return;
        }

        try {
            $asset = app(OrnamentAmazonService::class)->customizePreviewImage(
                user: auth()->user(),
                assetId: $this->assetId,
                target: $this->editTarget,
            ...
            app(ActivityLogService::class)->record(
                event: 'ornament_amazon.preview_image_customized',
                description: 'User customized an Ornament Amazon preview image.',
                subject: $asset,
                properties: ['item_number' => $asset->item_number, 'target' => $this->editTarget, 'provider' => $this->modalProviderKey],
            );
        } catch (RuntimeException $exception) {
            $this->reportUserActionError($exception, 'ornament_amazon.customize_preview_image', [
                'asset_id' => $this->assetId,
                'target' => $this->editTarget,
            ]);
            $this->dispatch('toast', type: 'error', title: 'Action failed!', message: $exception->getMessage());
            ...
            Log::error('Ornament Amazon preview image customization failed unexpectedly.', [
                'asset_id' => $this->assetId,
                'target' => $this->editTarget,
                'message' => $exception->getMessage(),
            ]);
    ...
    {
        return view('livewire.modals.ornament-amazon.review-image');
    }
    ...
    private function loadSourcePreviewContext(): void
    {
        ...
        if ($this->productSlug !== 'ornament' || ! $this->assetId || $this->title !== 'Source image') {
            return;
        }
