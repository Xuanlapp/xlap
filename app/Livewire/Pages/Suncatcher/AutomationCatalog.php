...
class AutomationCatalog extends Component
{
    ...
    #[Session(key: 'suncatcher-catalog.status')]
    public string $status = 'all';

    #[Session(key: 'suncatcher-catalog.search')]
    ...
    {
        $query = DataSuncatcher::query()->when(! auth()->user()->is_admin && ! auth()->user()->isManager(), fn (Builder $query) => $query->where('user_id', auth()->id()));
