...
class WorkflowActionButton extends Component
{
    ...
    public function run(): void
    {
        ...
        try {
            ...
            $this->dispatch('suncatcher-workflow-action-finished', assetId: $this->assetId, action: $this->action, person: $this->person);
    ...
    {
        return view('livewire.pages.suncatcher.workflow-action-button', [
    ...
    {
        try {
            $asset = app(SuncatcherService::class)->generateRedesign(
                auth()->user(),
                $this->assetId,
                $this->providerKey,
                $this->imageModel,
    ...
    {
        try {
            $asset = app(SuncatcherService::class)->generateWorkflowScript(
                auth()->user(),
                $this->assetId,
                $this->providerKey,
                $this->textModel,
    ...
    private function generatePerson(): void
    {
        ...
        try {
            $asset = app(SuncatcherService::class)->generateWorkflowPerson(
                auth()->user(),
                $this->assetId,
                $person,
                $this->providerKey,
    ...
    private function dispatchWorkflowUpdated(int $assetId): void
    {
        ...
        $this->dispatch('suncatcher-product-design-updated', assetId: $assetId);
    }
