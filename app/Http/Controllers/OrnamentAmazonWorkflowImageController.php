...
class OrnamentAmazonWorkflowImageController extends Controller
{
    public function redesign(Request $request, int $asset): JsonResponse
    {
        ...
        try {
            $freshAsset = app(OrnamentAmazonService::class)->generateRedesign(
                user: $request->user(),
                assetId: $asset,
            ...
            Log::error('Ornament Amazon parallel Create Image failed unexpectedly.', [
                'asset_id' => $asset,
                'message' => $exception->getMessage(),
            ]);
    ...
    public function script(Request $request, int $asset): JsonResponse
    {
        ...
        try {
            app(OrnamentAmazonService::class)->generateWorkflowScript(
                user: $request->user(),
                assetId: $asset,
                providerKey: $payload['provider_key'] ?? null,
            ...
            Log::error('Ornament Amazon parallel Generate Script failed unexpectedly.', [
                'asset_id' => $asset,
                'message' => $exception->getMessage(),
            ]);
    ...
    public function person(Request $request, int $asset, string $person): JsonResponse
    {
        ...
        try {
            $freshAsset = app(OrnamentAmazonService::class)->generateWorkflowPerson(
                user: $request->user(),
                assetId: $asset,
                person: $person,
                providerKey: $payload['provider_key'] ?? null,
            ...
            Log::error('Ornament Amazon parallel Person ref failed unexpectedly.', [
                'asset_id' => $asset,
                'person' => $person,
                'message' => $exception->getMessage(),
            ]);
    ...
    public function prepare(Request $request, int $asset): JsonResponse
    {
        try {
            ...
            Log::error('Ornament Amazon parallel B5 prepare failed unexpectedly.', [
                'asset_id' => $asset,
                'message' => $exception->getMessage(),
            ]);
    ...
    {
        try {
            $freshAsset = app(OrnamentAmazonService::class)->assetForUser($request->user(), $asset);
            $workflow = $this->workflowData($freshAsset->id);
            $images = [];
            ...
            Log::error('Ornament Amazon B5 status failed unexpectedly.', [
                'asset_id' => $asset,
                'message' => $exception->getMessage(),
            ]);
    ...
    public function generate(Request $request, int $asset, string $slot): JsonResponse
    {
        ...
        try {
            ...
            Log::error('Ornament Amazon parallel B5 slot failed unexpectedly.', [
                'asset_id' => $asset,
                'slot' => $slot,
                'message' => $exception->getMessage(),
            ]);
    ...
    private function ensureAssetCanGenerate(OrnamentAmazonService $service, Request $request, int $asset): void
    {
    ...
    {
        $workflowRecord = OrnamentAmazonWorkflow::query()
