...
class PsdMockupRenderer
{
    ...
    /**
     * Run one Glass PSD render at a time so weak VPS instances do not render multiple PSDs in parallel.
     *
    ...
    {
        return 'glass:psd-mockup-renderer:lock';
    }

    /**
     * Render the PSD once the global Glass render lock has been acquired.
     *
     * @return array<int, string>
     */
    private function renderUnlocked(PsdMockupTemplate $template, string $masterImageUri, int $assetId): array
    {
        ...
        $outputDirectory = Storage::disk('public')->path("generated/glass/mockups/{$assetId}");
    ...
    private function outputFiles(string $rendererOutput, string $outputDirectory, int $assetId): array
    {
        ...
        if ($outputs !== []) {
            return collect($outputs)
                ...
                ->map(fn (string $path): string => '/storage/generated/glass/mockups/'.$assetId.'/'.basename($path))
                ->take(11)
                ->values()
                ->all();
        }
        ...
        return collect(File::files($outputDirectory))
            ...
            ->values()
            ->take(11)
            ->map(fn (\SplFileInfo $file): string => '/storage/generated/glass/mockups/'.$assetId.'/'.$file->getFilename())
            ->all();
    }
