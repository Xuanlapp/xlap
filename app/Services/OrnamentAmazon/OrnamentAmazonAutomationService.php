...
class OrnamentAmazonAutomationService
{
    ...
    public function start(User $user, ProductDesignAsset $asset, array $payload = []): DataOrnamentAmazon
    {
        ...
        return DataOrnamentAmazon::query()->updateOrCreate(
            ...
            [
                'user_id' => $user->id,
                'product_slug' => $payload['product_slug'] ?? 'ornament-amazon',
                'workflow_name' => $payload['workflow_name'] ?? 'ornament-amazon-automation',
                'status' => 'running',
    ...
    public function syncStepResult(ProductDesignAsset $asset, string $step, bool $ok, ?string $error = null): DataOrnamentAmazon
    {
        $this->ensureAutomationTableExists();
    ...
    public function markStepRunning(ProductDesignAsset $asset, string $step): DataOrnamentAmazon
    {
        $this->ensureAutomationTableExists();
        $automation = $this->findForAsset($asset);
        $steps = $automation?->step_data ?: $automation?->steps ?: $this->defaultSteps();
    ...
    public function complete(ProductDesignAsset $asset): DataOrnamentAmazon
    {
        $this->ensureAutomationTableExists();
    ...
    public function fail(ProductDesignAsset $asset, string $step, string $message): DataOrnamentAmazon
    {
    ...
    public function resume(User $user, ProductDesignAsset $asset): void
    {
        ...
        GenerateOrnamentAmazonWorkflowImage::dispatch($user->id, $asset->id, $steps[$nextIndex], $automation->provider_key, $automation->image_model)
            ->onQueue('default');
    }

    public function findForAsset(ProductDesignAsset $asset): ?DataOrnamentAmazon
    {
        if (! Schema::hasTable('data_ornament_amazon')) {
            return null;
        }

        return DataOrnamentAmazon::query()->where('product_design_asset_id', $asset->id)->first();
    }
    ...
    private function writeAutomation(ProductDesignAsset $asset, array $attributes): DataOrnamentAmazon
    {
        $this->ensureAutomationTableExists();
        $automation = $this->findForAsset($asset);

        if (! $automation) {
            $automation = new DataOrnamentAmazon([
                'product_design_asset_id' => $asset->id,
