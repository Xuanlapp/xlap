...
class PsdMockupRenderer
{
    ...
    public function render(PsdMockupTemplate $template, string $masterImageUri, int $assetId): array
    {
        ...
        $outputDirectory = Storage::disk('public')->path("generated/suncatcher/mockups/{$assetId}");
    ...
    private function outputFiles(string $rendererOutput, string $outputDirectory, int $assetId): array
    {
        ...
        if ($outputs !== []) {
            return collect($outputs)
                ...
                ->map(fn (string $path): string => '/storage/generated/suncatcher/mockups/'.$assetId.'/'.basename($path))
                ->take(11)
                ->values()
                ->all();
        }
        ...
        return collect(File::files($outputDirectory))
            ...
            ->values()
            ->take(11)
            ->map(fn (\SplFileInfo $file): string => '/storage/generated/suncatcher/mockups/'.$assetId.'/'.$file->getFilename())
            ->all();
    }
