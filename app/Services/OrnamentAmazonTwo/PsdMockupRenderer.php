...
class PsdMockupRenderer
{
    ...
    public function render(PsdMockupTemplate $template, string $masterImageUri, int $assetId): array
    {
        ...
        $outputDirectory = Storage::disk('public')->path("generated/ornament-amazon-2/mockups/{$assetId}");
    ...
    private function outputFiles(string $rendererOutput, string $outputDirectory, int $assetId): array
    {
        ...
        if ($outputs !== []) {
            return collect($outputs)
                ...
                ->map(fn (string $path): string => '/storage/generated/ornament-amazon-2/mockups/'.$assetId.'/'.basename($path))
                ->take(11)
                ->values()
                ->all();
        }
        ...
        return collect(File::files($outputDirectory))
            ...
            ->values()
            ->take(11)
            ->map(fn (\SplFileInfo $file): string => '/storage/generated/ornament-amazon-2/mockups/'.$assetId.'/'.$file->getFilename())
            ->all();
    }
